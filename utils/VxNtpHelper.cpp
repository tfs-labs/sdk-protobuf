...
typedef enum em_ntp_mode_t
{
    ...
    ntp_mode_control    = 6,  ///<   NTP   The message is an NTP control packet
...
{
    x_uchar_t         xct_li_ver_mode;      ///< 2 bits， ；3 bits， ；3 bits，NTP （  em_ntp_mode_t  ）2 bits, leap indicator; 3 bits, version number; 3 bits, NTP operating mode (see em_ntp_mode_t Related enumeration values)
    x_uchar_t         xct_stratum    ;      ///<  ， 1~16， 。 1 ， 1 16 ， 16 ，   ， 1~16， 。 1 ， 1 16 ， 16 ， 
    x_uchar_t         xct_poll       ;      ///<  ， NTP  Polling time, which is the time interval between two consecutive NTP packets
    ...
    x_ntp_timestamp_t xtmst_reference;      ///<  （ ，  T1） The last time the system clock was set or updated (used to store T1 after the answer is completed)
    x_ntp_timestamp_t xtmst_originate;      ///< NTP （ ，  T4）  The local time of the sender when the NTP request packet leaves the sender (after the reply is completed, it is used to store T4)
    x_ntp_timestamp_t xtmst_receive  ;      ///< NTP （ ，  T2） The local time of the receiver when the NTP request packet arrives at the receiver (after the reply is completed, it is used to store T2)
    x_ntp_timestamp_t xtmst_transmit ;      ///< NTP （ ，  T3） The local time of the responder when the NTP reply message leaves the responder (used to store T3 after the reply is complete.)
} x_ntp_packet_t;
...
/**********************************************************/
/**
 * @brief   x_ntp_timeval_t   x_ntp_timestamp_t 。
...
x_void_t ntp_gettimeofday(x_ntp_timeval_t * xtm_value)
{
    ...
    xtm_value->tv_sec  = (x_long_t)((xtime_value.QuadPart - NS100_1970) / 10000000LL); // 1970 
...
/**********************************************************/
/**
 * @brief   x_ntp_time_context_t     100 
 *         （1970 1 1 ）。
 ...
 *
 * @param [in ] xut_time    :  （1970 1 1 ）。 Time value (time from January 1, 1970 to the present)
 ...
 * @param [out] xut_value :   X_NULL， ，  IP  。 If the input parameter is not X_NULL, the corresponding IP address value is returned when the operation is successful.
 *
 * @return x_bool_t
 *         -  ，  X_TRUE；
 *         -  ，  X_FALSE。
...
/**********************************************************/
/**
 * @brief     IP  （  gethostbyname() API  ）。
 * Get a table of IP addresses under the domain name (instead of the system's gethostbyname() API call)
 *
 * @param [in ] xszt_dname :  （ ：www.163.com）。The specified domain name (in the format: www.163.com).
 ...
 * </pre>
 *
 * @param [in ] xszt_host : NTP   IP（  IP  ）。
 *                          The IP of the NTP server (four-segment IP address).
 * @param [in ] xut_port  : NTP    （  NTP_PORT : 123）。The port number of the NTP server (whichever is the default port number NTP_PORT : 123).
 * @param [in ] xut_tmout :  （   ）。Timeout (in milliseconds)
 * @param [out] xit_tmlst :  （T1、T2、T3、T4）。The timestamp required for the computation returned by the operation successfully (T1, T2, T3, T4).
 *
 * @return x_int32_t
 *         -  ，  0；
 *         -  ，   。
...
/**********************************************************/
/**
 * @brief   NTP   NTP  ， 。 Send an NTP request to the NTP server to get the server timestamp.
 * 
 * @param [in ] xszt_host : NTP   IP（  IP  ）    （  3.cn.pool.ntp.org）。 The IP (four-segment IP address) or domain name (such as 3.cn.pool.ntp.org) of the NTP server.
 * @param [in ] xut_port  : NTP    （  NTP_PORT : 123）。  Port number of the NTP server (default port number NTP_PORT : 123)
 * @param [in ] xut_tmout :  （ ）。 Timeout for network requests in milliseconds
 * @param [out] xut_timev :  （  100   ，1970 1 1 ）。The answer time value returned by the successful operation in 100 nanoseconds from January 1, 1970 to the present.
 * 
 * @return x_int32_t
 *         -  ，  0；
 *         -  ，   。
...
/**********************************************************/
/**
 * @brief   NTP   NTP  ， ， 。 Send an NTP request to the NTP server to get the server timestamp for testing.
 * 
 * @param [in ] xszt_host : NTP   IP（  IP  ）    （  3.cn.pool.ntp.org）。The IP (four-segment IP address) or domain name (such as 3.cn.pool.ntp.org) of the NTP server.
 * @param [in ] xut_port  : NTP    （  NTP_PORT : 123）。The port number of the NTP server (whichever is the default port number NTP_PORT : 123).
